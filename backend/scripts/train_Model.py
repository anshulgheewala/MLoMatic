...
#     categorical_cols = [col for col in X.columns if X[col].dtype == 'object' or X[col].nunique() < 7]
...
#     # Remove commas from numeric strings if any
#     for col in categorical_cols:
#         try:
...
#     # Apply SMOTE for classification only
#     if problem_type == 'classification':
...
#     # Parameter grids (unchanged)
#     classification_params = {
#         'Logistic Regression': {'C': [0.01, 0.1, 1, 10], 'solver': ['lbfgs', 'liblinear']},
#         'Random Forest': {'n_estimators': [100, 200], 'max_depth': [5, 10], 'min_samples_split': [2, 5], 'min_samples_leaf': [1, 2]},
...
#         'Decision Tree': {'criterion': ['gini', 'entropy'], 'splitter': ['best', 'random'], 'max_depth': [None, 10, 20, 30, 40, 50], 'min_samples_split': [2, 5, 10], 'min_samples_leaf': [1, 2, 4], 'max_features': [None, 'sqrt', 'log2']}
#     }
...
#         'Random Forest': {'n_estimators': [100, 200], 'max_depth': [5, 10], 'min_samples_split': [2, 5], 'min_samples_leaf': [1, 2]},
#         'Support Vector Machine': {'C': [0.1, 1, 10], 'kernel': ['linear', 'rbf'], 'gamma': ['scale', 'auto']},
#         'Decision Tree': {'criterion': ['squared_error', 'friedman_mse', 'absolute_error'], 'splitter': ['best', 'random'], 'max_depth': [None, 10, 20, 30, 40, 50], 'min_samples_split': [2, 5, 10], 'min_samples_leaf': [1, 2, 4], 'max_features': [None, 'sqrt', 'log2']}
#     }
...
#         if params:
#             grid = GridSearchCV(model, param_grid=params, cv=5, scoring=scoring_metric, n_jobs=3)
#             grid.fit(X_train, y_train)
#             best_estimator = grid.best_estimator_
#             best_params = grid.best_params_
#             cv_score = grid.best_score_
...
#             f1 = f1_score(y_test, y_pred, average='weighted', zero_division=0)
#         else:
#             cm, precision, recall, f1 = None, None, None, None
